...
EDITS AUTHOR: Tyler Taormina
05.31.2021
Credits: M Young, January 2019
...
#FIXME double check print/read gen
...
class If(Control):
    ...
    def gen(self, context: "Context", target: str):
        """ if/else/fi"""
        else_part = context.new_label("else") # make labels
        fi_part = context.new_label("fi")
        self.cond.condjump(context, target, else_part, jump_cond=False) # if true, do thenpart, otherwise do elsepart
        self.thenpart.gen(context, target)
        context.add_line(f"  JUMP  {fi_part}") # skip else part and go to fi part
        context.add_line(f"{else_part}:") # jump here if jump_cond = False
        self.elsepart.gen(context, target)
        context.add_line(f"{fi_part}:") # end
