...
class Tile(Listenable):
    ...
    def __repr__(self):
        """Looks like a call to create a tile"""
...
class Board(object):
    ...
    def solve(self) -> bool:
        """General solver involving constraint
        propagation (naked and hidden single techniques)
        and a guess-and-check method
        """
        self.propagate()
        if self.is_complete():
            return True
            ...
            saved = self.as_list()
            min = self.min_choice_tile()
            for value in min.candidates:
                min.set_value(value)
                if self.solve():
                    return True
                else:
    ...
    def is_consistent(self) -> bool:
        """Check for duplicate values within a row, column, or block"""
        for group in self.groups:
            used_set = set()
            for tile in group:
                if tile.value != UNKNOWN:
                    if tile.value in used_set:
                        return False
                    else:
                        used_set.add(tile.value)
        return True
    ...
    def naked_single(self) -> bool:
        ...
        for group in self.groups:
            used_sym = set()
            for sym in group:
                used_sym.add(sym.value)
            for sym in group:
                changed = sym.remove_candidates(used_sym)
                if changed:
                    ct += 1
        return ct > 0
        ...
        for group in self.groups:
            for tile in group:
                if tile.value is UNKNOWN:
                    if len(tile.candidates) < compare:
                        min_choice = tile
                        compare = len(min_choice.candidates)
        return min_choice
