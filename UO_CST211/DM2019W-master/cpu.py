...
class CPU(MVCListenable):
    ...
    def step(self):
        ...
        # fetch
        addr = self.registers[15].get()
        instr_word = self.memory.get(addr)
        ...
        if instr_predicate > 0:  # perform specified operation
            left_operand = self.registers[instr.reg_src1].get()
            src2 = self.registers[instr.reg_src2].get()
            right_operand = instr.offset + src2
            result_value, result_flag = self.alu.exec(instr.op, left_operand, right_operand)
            reg_increment = self.registers[15].get()
            self.registers[15].put(reg_increment + 1)
            if instr.op == OpCode.STORE:
                self.memory.put(result_value, self.registers[instr.reg_target].get())
            elif instr.op == OpCode.LOAD:
                item = self.memory.get(result_value)
                self.registers[instr.reg_target].put(item)
            elif instr.op == OpCode.HALT:
                self.halted = True
            else:
                self.registers[instr.reg_target].put(result_value)
                self.condition = result_flag
