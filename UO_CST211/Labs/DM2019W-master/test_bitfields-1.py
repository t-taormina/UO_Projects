...
class Test_Non_Interference(unittest.TestCase):

    def test_extract_mid(self):
        mid_bits = BitField(4, 6)  # 011
        high_bits = BitField(7, 8)  # 01
        field_bits = 0b100110011

        self.assertEqual(mid_bits.extract(field_bits), 0b011)
        self.assertEqual(high_bits.extract(field_bits), 0b10)
        self.assertEqual(0b100110011, field_bits)

    def test_insert_mid(self):
        mid_bits = BitField(3, 5)  # 011
        field_bits = 0b_000_000_011

        self.assertEqual(mid_bits.insert(0b011, field_bits), 0b_000_011_011)
