"""
CIS 211 Project: Contagion
Author: Tyler Taormina
Credits: Avery Meyers
Date: 04-19-2021

This file A SIR model for a contagious disease.
The numerical parameters of the disease have been selected merely to provide
interesting, somewhat realistic behavior, and do not reflect
actual numbers (e.g., infection rates, incubation periods,
recovery times and rates) for any particular disease. It is
an illustration, not a real epidemiological model. SIR models can be conveniently
modeled by a main loop in which a simulated clock or calendar moves
forward in regular increments. These regular increments, or
"ticks" of the virtual clock, provide an approximation of continuous
time in which many individuals may be evolving concurrently. To
maintain an illusion that all individuals evolve simultaneously,
each simulation cycle (tick) is divided into two parts. In the first
part, a next state for each individual is computed, using only the current
states of other individuals. (Thus the order in which we loop through
individuals does not matter.) In the second part, each individual
advances to its next state (and again the order in which we loop
through individuals doesn't matter).
"""
...
class Typical(Individual):
    ...
    def hello(self, visitor: "Individual") -> bool:
        return random.random() <= self.P_Greet
...
class AtRisk(Individual):
    ...
    def hello(self, visitor: "Individual") -> bool:
        for address in self.neighbors:
            if visitor == self.region.visit(address):
                return random.random() <= self.P_Greet
        return False


class Wanderer(Individual):
    "Individual who ignores travel restrictions and visitation guidelines"
